...
const Navbar = () => {
  ...
  const categories = [
    ...
    { name: "사회활동", path: "/social", activities: ["봉사", "교내모임"] },
  ];
  ...
  {categories.map((category, index) => (
    <div key={index} 
      ...
      <button
        ...
        className="text-gray-600 hover:text-blue-500 px-4 py-2 w-32"
      >
        {category.name}
      </button>
      {activeDropdown === category.name && (
        <div className="absolute top-full left-0 w-32 bg-white shadow-md">
          {category.activities.map((activity, activityIndex) => (
            // 하위 항목 선택 시 상위 항목의 페이지로 이동
            <Link
              key={activityIndex}
              to={category.path}
              className="block px-4 py-2 hover:bg-gray-100 text-gray-600 text-center"
            >
              {activity}
            </Link>
          ))}
